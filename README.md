...
<p align="center">轻量 Windows 程序，屏幕被关闭、或者合上盖子时自动锁屏。</p>
...
如需开机自启，右键托盘图标勾选「开机启动」即可。

## 功能

为什么做这个程序？

Windows 笔记本在运行任务时，可能需要关闭屏幕或合上盖子，同时不希望中断正在进行的任务。虽然可以把电源按钮设为「关闭显示器」、合盖操作设为「不采取任何操作」，但这样只是关闭了屏幕，重新点亮时无需输入密码，安全性不足。

如果设置为睡眠，虽然能够锁屏，但代价是破坏性的：睡眠会中止正在运行的任务。

因此有了这个小程序：让 Windows 笔记本在关闭屏幕或合上盖子时自动锁屏。
...
- 右键菜单：
  ...
  | 延迟锁定 | 子菜单，单选：立即 / 3 秒 / 5 秒 / 10 秒 / 30 秒。息屏或合盖后延迟相应秒数再锁定；期间重新亮屏或开盖则取消本次锁定 |
  | —— 分割线 —— | |
  | 开机启动 | 勾选后随系统登录自动启动；取消时会把注册表里那一项彻底删掉 |
  | 电源设置 | 检查电源计划里的电源按钮 / 合盖动作是否符合推荐值，并支持一键修正 |
  | —— 分割线 —— | |
  | 检查更新 | 与「关于」窗口中的同名按钮功能一致：检查是否有新版本 |
  | 关于 | 一句话说明、作者、项目地址与许可证（后两个可点击）、版本、注册表位置；另有语言按钮与「检查更新」按钮 |
  | 退出 | 移除托盘图标并结束进程 |

- **多语言**：界面支持 English / 简体中文。首次运行时会先弹出语言选择窗口，选择
  结果记入注册表，之后不再询问；如需更换语言，可在「关于」窗口中点击
  「Language / 语言」重新选择（更换后已打开的对话框会关闭，重新打开即为新语言）。
- 设置保存在注册表 `HKCU\Software\ScreenOffLock`，全是 DWORD：
  ...
  - `SkippedVersion` —— 「忽略当前版本」记下的远端版本号（默认不写）
...
- 完全卸载：取消勾选「开机启动」→ 退出程序 → 删除 exe →
  删除注册表项 `HKCU\Software\ScreenOffLock`。
...
本程序生效有个前提：**屏幕确实被关闭、盖子状态由系统上报**。因此托盘菜单中的
「电源设置」会读取当前电源计划里这两项的取值：
...
拿 `APP_VERSION` 里 Build 后面那段数字，跟 GitHub 上最新 Release 的 tag 比大小
（tag 就写成 `1400` 这种纯数字，所以发版时 tag 跟 build 号保持一致即可）。

- **启动后静默检查一次**：只有确实存在新版本时才弹提示。检查失败会重试，最多 3
  次，仍然失败则静默放弃，不会有任何报错提示。「忽略当前版本」勾选框会把本次看到
  的远端版本号记入 `SkippedVersion`，之后不再为这个版本提示；待线上发布更高的版本，
  提示会重新出现。
- **托盘菜单和「关于」窗口中都有「检查更新」**：点击后弹出检查窗口，先显示
  「正在检查更新」（可取消），结果返回后原地变为「发现新版本：Build N / 当前已是
  最新 / 更新检查失败」，按钮随之变为「前往更新 / 完成 / 关闭」。点击「前往更新」
  会用默认浏览器打开 Release 页面。
- **两个窗口中都会显示 Release 的说明正文**（即发版时写的那段内容）。如果说明中用
  一条单独成行的 `---` 分隔中英文，界面为中文时只显示中文段、英文同理——按两段中
  中文字符的占比自动判断，不依赖书写顺序。Markdown 标记会被去除：`#`、`**`、`*`、
  `` ` `` 之类的符号直接删除，链接只保留显示文字，图片替换为一句
  「请前往更新页面查看图片」。窗口高度会随说明长度变化。
- 请求通过 WinHTTP 发出，但**函数是运行时从 `winhttp.dll` 中获取的**，因此静态
  导入表仍为原来那六个系统 DLL，没有增加网络方面的硬依赖；`winhttp.dll` 不存在时
  该功能自动失效，其它功能不受影响。请求在工作线程中执行，不会阻塞托盘菜单。
- 只有「检查更新」相关的操作会联网，程序平时不发出任何请求。
...
存在，随意编造一个 GUID 也会返回成功句柄，拿它的返回值当依据会出错。
...
- **不覆盖台式机“直接关掉显示器电源”的场景**。物理断电会让显示器从系统拓扑里
  ...
  察觉不到。这部分留待后续版本。
- 托盘图标由 `scripts\make-icon.ps1` 生成。`src\app.ico` 已入库。它仍是占位性质，
  可随时替换。
