...
Windows 笔记本跑任务时，想关屏或合盖，又不想中断正在跑的东西。虽然可以把电源按钮设置成关闭显示器、合上盖子设置为无操作，但是也就仅仅是关闭屏幕了，亮屏并不需要重新输入密码，很没有安全性。
而设置成睡眠的话，虽然能锁屏，但是是破坏性的，睡眠会停掉后台还跑着的东西。

因此，这个小软件诞生了。可以让 Windows 笔记本关闭屏幕或是合上盖子时，自动锁屏。
...
- 右键菜单：
  ...
  | —— 分割线 —— | |
  | 检查更新 | 与「关于」里那个同名按钮功能一致：查一下有没有新版本 |
...
- **启动后静默查一次**：只有确实有新版本才弹提示。检查失败会重试，最多 3 次，
  仍然失败就彻底安静——不会有任何报错打扰。「忽略当前版本」这个勾选框会把这次
  看到的远端版本号记进 `SkippedVersion`，之后不再为这个版本提示；等线上出了更高
  的版本，提示会自己回来。
- **托盘菜单和「关于」窗口里都有「检查更新」**：点了弹出检查窗口，先显示「正在检查更新」
  （可以取消），结果回来后就地变成「发现新版本：Build N / 当前已是最新 / 更新检查失败」，
  按钮跟着变成「前往更新 / 完成 / 关闭」。点「前往更新」会用默认浏览器打开
  Release 页面。
- **两个窗口里都会显示 Release 的说明正文**（就是发版时写的那段）。如果说明里用
  一条单独成行的 `---` 把中英分开，界面是中文就只显示中文那段、英文同理——按两段
  里中文字符的占比自动判断，不依赖你写的先后顺序。Markdown 标记会被去掉：`#`、
  `**`、`*`、`` ` `` 之类的符号直接抹掉，链接只留显示文字，图片换成一句
  「请前往更新页面查看图片」。窗口高度会跟着说明长短走。
- 请求走 WinHTTP，但**函数是运行时从 `winhttp.dll` 里取的**，所以静态导入表还是
  原来那六个系统 DLL，没有多出网络方面的硬依赖；`winhttp.dll` 不存在时这个功能
  自动失效，其它功能照常。请求跑在工作线程上，不会卡住托盘菜单。
- 只有「检查更新」相关的动作会联网，平时程序不发出任何请求。
