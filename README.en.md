...
A Windows laptop running a long task may need its screen turned off or its lid closed,
without interrupting that task. The power button can be set to "Turn off display" and the
lid close action to "Do nothing", but that only turns the display off — waking it up asks
for no password, so there is no real security.

Sleeping does lock the screen, but the cost is destructive: sleep suspends whatever is
still running.

Hence this small program: it locks the screen automatically when a Windows laptop's display
is turned off or the lid is closed.
...
- Complete removal: untick **Run at startup** → exit the program → delete the exe → delete
...
- **A silent check runs at startup.** A prompt appears only when a newer version really
  exists. A failed check is retried, up to 3 times, and then abandoned silently, with no
  error message. Ticking "Skip this version" records the remote version number in
  `SkippedVersion`, so that version is not offered again; once a higher version is
  published, the prompt returns.
...
- The **About window** does not use `MessageBox`: its icon can only be one of the old
  ...
  glyph (`U+E946` ⓘ), and the buttons stay standard system buttons to preserve the native
  look and feel.
...
- **Desktops that simply cut power to the monitor are not covered.** A physical power-down
  ...
  future version.
- The tray icon is generated by `scripts\make-icon.ps1`. It is committed as `src\app.ico`.
